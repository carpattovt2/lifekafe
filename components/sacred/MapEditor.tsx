...
function getIllusSVG(ill: IllustrationObj): string {
  if (ill.kind==='mountain') return MOUNTAIN_SVGS[ill.variant % MOUNTAIN_SVGS.length]
  if (ill.kind==='forest')   return FOREST_SVGS[ill.variant % FOREST_SVGS.length]
...
export default function MapEditor() {
  ...
  function onMouseDown(e:React.MouseEvent) {
    ...
    if(tool==='illustrate'){
      ...
      const numVariants=kind==='mountain'?4:kind==='forest'?4:1
      const v=(illVariants.current[kind]??0)%numVariants
      illVariants.current[kind]=(v+1)%numVariants
      const baseAngle=(kind==='mountain'||kind==='forest')?(Math.random()-0.5)*18:0
      const newIll:IllustrationObj={id:crypto.randomUUID(),kind,variant:v,x,y,size:illusSzRef.current,angle:baseAngle}
